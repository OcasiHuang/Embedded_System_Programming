...
#define DRIVER_NAME1 "/dev/dataqueue1"
#define DRIVER_NAME2 "/dev/dataqueue2"
#define BASE_PERIOD 1000
#define SIZE 10
//#pragma intrinsic(__rdtsc)  

//Sorry, we are unable to use dynamic allocation this time...:( .by YuChun Huang
double items1[SIZE+2];//queue 1, [10] for rptr, [11] for wptr
double items2[SIZE+2];//queue 2, [10] for rptr, [11] for wptr
double * items[2];
...
void sq_write(double* it, double element)//write queue
{
	...
	//return(-1);
        ...
        it[(int)it[SIZE+1]] = element;
	if(it==&items1[0]){
		i1_write_t[(int)it[SIZE+1]] = rdtsc();
		printf("TIME Q1 = %"PRIu64"\n",i1_write_t[(int)it[SIZE+1]]);
	}else{
		i2_write_t[(int)it[SIZE+1]] = rdtsc();
		printf("TIME Q2 = %"PRIu64"\n",i2_write_t[(int)it[SIZE+1]]);
	}
        it[SIZE+1] = (int)(it[SIZE+1] + 1) % SIZE;
    }
}
...
double sq_read(double* it)//read queue
{
    ...
    if(isEmpty(it)) {
        ...
        element = it[(int)it[SIZE]];
	if(it==&items1[0]){// count for TSC	
		temp = rdtsc();
		temp -= i1_write_t[(int)it[SIZE]];
		...
		temp = rdtsc();
		temp -= i2_write_t[(int)it[SIZE]];
		printf("TIME interval Q2 = %"PRIu64"\n",temp);
 		num_i2 += 1;
 		total_i2 += temp;
 		e2_i2 += pow(temp,2);
	}
        if (((int)(it[SIZE]+1)%SIZE) == (int)it[SIZE+1]){
            it[SIZE] = -1;
            it[SIZE+1] = 0;
        } 
        else {
            it[SIZE] = (int)(it[SIZE] + 1) % SIZE;
...
void *thread_receiver(void *data)//receiver thread with period 40*PERIOD
{
	...
	while (!END){
		...
		switch(queue_sel){
		case 0:
			while(!isEmpty(items1)){
				fp = open(DRIVER_NAME1, 0);	//open driver, read only
				
				//num = read(fp, &items1[(int)items1[SIZE]], 1);
				num = read(fp, &str, 8);
				if (num == 0) printf("READ FAILED\n");
				printf("read string %s\n",str);
				ret = strtod(str,NULL);
				//printf("RET!!! = %lf, num = %d\n", ret, num);
				items1[(int)items1[SIZE]] = ret;
				value = sq_read(&items1[0]);//now data is copied to items1, we can use the original function call

				printf("Dequeue value = %lf\n", value);
			}
			break;
		case 1:
			while(!isEmpty(items2)){
				fp = open(DRIVER_NAME2, 0);	//open driver, read only

				//num = read(fp, &items2[(int)items2[SIZE]], 1);
				num = read(fp, &str, 8);
				if (num == 0) printf("READ FAILED\n");
				printf("read string %s\n",str);
				ret = strtod(str,NULL);
				//printf("RET!!! = %lf, num = %d\n", ret, num);
				items2[(int)items2[SIZE]] = ret;
				value = sq_read(&items2[0]);//now data is copied to items1, we can use the original function call
...
void *thread_period(void *data)//periodic thread, with 4 different multiplier for PERIOD 
{
	...
	while (!END){
		...
		if(period_multiplier == 12 || period_multiplier == 32){	
			...
			//printf("pi is %lf, write at %d\n",items1[(int)items1[SIZE+1]-1], (int)items1[SIZE+1]-1);
			//printf("1. pi is %lf\n",pi);

			fp = open(DRIVER_NAME1, O_RDWR);
			sprintf(s,"%1.6f",pi);
			printf("string %s\n",s);
			write(fp, &s[0], 8);
			//num = write(fp, &s[0], 8);
			//if (num == 0) printf("WRITE FAILED");
		}else if(period_multiplier == 18 || period_multiplier == 28){	
			sq_write(&items2[0],pi);
			//printf("pi is %lf, write at %d\n",items2[(int)items2[SIZE+1]-1], (int)items2[SIZE+1]-1);
			//printf("2. pi is %lf\n",pi);

			fp = open(DRIVER_NAME2, O_RDWR);
			sprintf(s,"%1.6f",pi);
			printf("string %s\n",s);
			write(fp, &s[0], 8);
			//num = write(fp, &s[0], 8);
		...
		if(ie.code == 272 && ie.value == 1){	
			if(click_count != 0)
			{
				...
				if(result <= 500000){
					...
					goto END;
      //while(1){
      	//if(num_i1 >= 10 && num_i2 >= 10) goto END;
      //}
				}
			}else{
				click_count++;
